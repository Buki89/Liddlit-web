...
const Svg = styled("svg")<{ rotated?: boolean }>`
  transform: ${({ rotated }) => (rotated ? "rotate(180deg)" : "")};
  :hover {
    fill: ${({ rotated }) => (rotated ? "#7193ff" : "#f94502")};
  }
`;

const fillColor = (rotated: boolean, voted: boolean): string => {
  if (rotated && voted) {
    return "#7193ff";
  }
  if (!rotated && voted) {
...
const ArrowIcon: FC<ArrowIconProps> = ({ rotated = false, color, voted }) => {
  return (
    <Svg
      version="1.1"
      id="Capa_1"
